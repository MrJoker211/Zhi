...
public class SecondChapter extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.second_chapter);
        //初始化控件
        initView();
        //设置控件点击监听
        setClick();
                    ...
                    bmobQuery.getObject(object.get(mCurrentIndex).getObjectId(), new QueryListener<ChoiceTable>() {
                        ...
                        public void done(ChoiceTable choiceTable, BmobException e) {
                            if (e == null) {
                                ...
                                Toast.makeText(SecondChapter.this, "加载失败", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                } else {
                    Toast.makeText(SecondChapter.this, "查询ObjectId失败" + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.sure:
                ...
                //如果都未被选中先弹出请选择
                if ((!checkBoxA.isChecked()) && (!checkBoxB.isChecked()) && (!checkBoxC.isChecked()) && (!checkBoxD.isChecked())) {
                    Toast.makeText(SecondChapter.this, "请您选择一个答案！", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.last_question:
                ...
                //首先判断上一题是不是空
                if (mCurrentIndex == 0) {
                    Toast.makeText(SecondChapter.this, "当前内容为第一题！", Toast.LENGTH_SHORT).show();
                } else {
                    //将答案栏隐藏,并清除选项框的选中状态
                    checkBoxA.setChecked(false);
                    checkBoxB.setChecked(false);
            ...
            case R.id.next_question:
                ...
                if (mCurrentIndex == (mTableLength-1)) {
                    Toast.makeText(SecondChapter.this, "当前内容为最后一题！", Toast.LENGTH_SHORT).show();
                } else {
                    //将答案栏隐藏,并清除选项框的选中状态
                    checkBoxA.setChecked(false);
                    checkBoxB.setChecked(false);
                        ...
                        public void done(ChoiceTable choiceTable, BmobException e) {
                            if (e == null) {
                                ...
                                if (choiceTable.getAnswer().equals(checkAnswer)) {
                                    Toast.makeText(SecondChapter.this, "回答正确，干得漂亮！", Toast.LENGTH_SHORT).show();
                                } else {
                                    Toast.makeText(SecondChapter.this, "做错了，再接再厉！", Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Toast.makeText(SecondChapter.this, "加载答案失败", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                } else {
