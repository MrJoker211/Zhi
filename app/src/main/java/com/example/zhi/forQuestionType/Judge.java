package com.example.zhi.forQuestionType;
...
public class Judge extends AppCompatActivity implements View.OnClickListener {
                    ...
                    bmobQuery.getObject(object.get(mCurrentIndex).getObjectId(), new QueryListener<JudgeTable>() {
                        ...
                        public void done(JudgeTable judgeTable, BmobException e) {
                            if (e == null) {
                                ...
                                Toast.makeText(Judge.this, "加载失败", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                } else {
                    Toast.makeText(Judge.this, "查询ObjectId失败" + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.sure:
                ...
                if((!checkBoxWrong.isChecked())&&(!checkBoxRight.isChecked())){
                    Toast.makeText(Judge.this,"请您选择一个答案！",Toast.LENGTH_SHORT).show();
                }
            ...
            case R.id.last_question:
                ...
                //首先判断上一题是不是空
                if (mCurrentIndex == 0) {
                    Toast.makeText(Judge.this, "当前内容为第一题！", Toast.LENGTH_SHORT).show();
                } else {
                    //将答案栏隐藏,并清除选项框的选中状态
                    checkBoxRight.setChecked(false);
                    checkBoxWrong.setChecked(false);
            ...
            case R.id.next_question:
                ...
                if (mCurrentIndex == (mTableLength-1)) {
                    Toast.makeText(Judge.this, "当前内容为最后一题！", Toast.LENGTH_SHORT).show();
                } else {
                    //将答案栏隐藏,并清除选项框的选中状态
                    checkBoxRight.setChecked(false);
                    checkBoxWrong.setChecked(false);
                        ...
                        public void done(JudgeTable judgeTable, BmobException e) {
                            if (e == null) {
                                ...
                                if(judgeTable.getAnswer().equals(checkAnswer)){
                                    Toast.makeText(Judge.this, "回答正确，干得漂亮！", Toast.LENGTH_SHORT).show();
                                }else{
                                    Toast.makeText(Judge.this, "做错了，再接再厉！", Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Toast.makeText(Judge.this, "加载失败", Toast.LENGTH_SHORT).show();
                            }
